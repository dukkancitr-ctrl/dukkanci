...
const REPLY_ORDER = `🛒 الطلب من دكانجي بخطوات بسيطة:\n1) افتح ${SITE_URL} واختر المتجر (السلة تكون من متجر واحد في كل طلب).\n2) أضِف منتجاتك وأدخل عنوانك ورقم واتساب.\n3) سيصلك رمز تأكيد على واتساب — أدخله لإرسال الطلب للمتجر.\nالدفع عند الاستلام (نقداً أو بالبطاقة عبر جهاز POS) أو بتحويل بنكي حسب ما يتيحه المتجر.\nلا تعرف ماذا تطلب؟ جرّب «اسأل دكانجي»: ${SITE_URL}/ask-dukkanci 🌟`;
const REPLY_TRACK = `📦 متابعة طلبك:\n• افتح «طلباتي» في الموقع لترى حالته لحظة بلحظة (طلب جديد ← قيد التجهيز ← خرج للتوصيل ← تم التسليم).\n• تصلك رسالة واتساب عند كل تغيّر في الحالة.\n• يمكنك إلغاء الطلب من «طلباتي» قبل خروجه للتوصيل.\n• طلبك متأخر أو لم تصلك تحديثات؟ أرسل لنا رقمه (مثل DK-1234567) وسيتابعه فريقنا فوراً. 🚚`;
const REPLY_DELIVERY = `🚚 التوصيل في دكانجي:\n• تُحسب الرسوم حسب المسافة بين المتجر وعنوانك، وتراها بوضوح (مع طريقة الحساب) قبل تأكيد الطلب.\n• بعض المتاجر لها أسعار ثابتة لمناطق محدّدة أو توصيل مجاني فوق حدّ معيّن.\n• نخدم أحياء إسطنبول، ويختلف نطاق التوصيل ووقته من متجر لآخر. لمعرفة المتاجر القريبة منك: ${SITE_URL}/stores\n• الاستلام من المتجر بنفسك مجاني. 🏪`;
const REPLY_OFFERS = `🎁 عروض وخصومات حقيقية يضعها التجار أنفسهم — تجدها كلها في صفحة العروض:\n${SITE_URL}/offers\nتتغيّر باستمرار كلما حدّث التاجر أسعاره، فمرّ عليها قبل كل طلب. ✨`;
const REPLY_STORES = `🏪 تصفّح كل المتاجر المعتمدة على دكانجي (مطاعم، حلويات، سوبرماركت، ملاحم وغيرها):\n${SITE_URL}/stores\nتريد الأقرب أو الأعلى تقييماً؟ استخدم «دليل دكانجي» بتقييمات Google ومقارنة المتاجر: ${SITE_URL}/dalil 🛍️`;
const REPLY_SUPPORT = `💬 نحن هنا لمساعدتك! اكتب مشكلتك أو استفسارك هنا في رسالة واحدة (ويُفضَّل ذكر رقم الطلب إن وُجد) وسيردّ عليك فريق دعم دكانجي خلال ساعات العمل (يومياً ٩ صباحاً–١١ مساءً بتوقيت إسطنبول).\nأو تصفّح الأسئلة الشائعة: ${SITE_URL}/faq`;
...
const REPLY_MERCHANT = `🤝 يسعدنا انضمامك كتاجر في دكانجي!\n✔ بلا عمولة على الطلبات\n✔ إشعار فوري بكل طلب + لوحة تحكم بمنتجاتك وأسعارك\n✔ صفحة متجر خاصة بك مفهرسة على غوغل\nللتفاصيل وإضافة متجرك تواصل مباشرة مع فريق التجار على واتساب: +90 552 800 02 20 📞\nأو اطّلع على المزايا: ${SITE_URL}/merchants`;
...
const REPLY_WELCOME = `أهلاً بك في دكانجي! 🛍️ سوق الحي بين يديك — متاجر ومطاعم وبقالات حيّك في إسطنبول.\nكيف نساعدك؟ اختر أو اكتب:\n• /order — كيف أطلب\n• /track — أين وصل طلبي\n• /delivery — التوصيل والرسوم\n• /offers — العروض\n• /support — التحدث مع الدعم\nأو اكتب سؤالك مباشرة (مثلاً «عندكم مطاعم؟» أو اسم منتج) وسأجيبك، وإن أردت موظفاً اكتب «موظف». 🌟`;
const REPLY_AWAY = `شكراً لتواصلك مع دكانجي! 🌙\nفريق الدعم خارج أوقات العمل حالياً (يومياً ٩ صباحاً–١١ مساءً بتوقيت إسطنبول) وسنردّ عليك فور بدء الدوام — اترك رسالتك هنا مع رقم طلبك إن وُجد.\nوفي الأثناء الموقع يعمل على مدار الساعة للطلب وتتبّع الطلبات من «طلباتي»: ${SITE_URL}`;
...
const REPLY_VOICE = `🎙️ وصلتنا رسالتك الصوتية، لكن لا يمكنني الاستماع للرسائل الصوتية حالياً.\nاكتب استفسارك نصاً (مثلاً: «أين طلبي DK-1234567» أو «عندكم مطعم شاورما؟») وسأجيبك فوراً، أو اكتب «موظف» ليتواصل معك أحد فريقنا. 🙏`;
...
const REPLY_IOS = `📱 دكانجي على الآيفون:\nتطبيقنا على المتجر متاح حالياً لأندرويد فقط (Google Play)، لكن الموقع يُثبَّت على الآيفون كتطبيق كامل بأيقونته وبلا حاجة لـApp Store:\n1) افتح ${SITE_URL} من متصفح Safari.\n2) اضغط زر المشاركة (المربع والسهم للأعلى).\n3) اختر «إضافة إلى الشاشة الرئيسية» ثم «إضافة».\nوستصلك رسائل الطلبات على واتساب كالمعتاد. 🍏`;
...
const ESCALATION_ACK_REPLY = {
  delete_account: "تلقّينا طلبك بحذف الحساب 🗑️\nحذف الحساب إجراء نهائي لا يمكن التراجع عنه، لذلك ينفّذه أحد موظفينا يدوياً بعد التأكد من هويتك. سيتواصل معك قريباً خلال ساعات العمل (٩ صباحاً–١١ مساءً بتوقيت إسطنبول).",
  cancel: "استلمنا طلبك 🙌 وحوّلناه لموظف من فريق دكانجي ليتابع معك.\nلتسريع الأمر أرسل رقم الطلب (مثل DK-1234567) وسبب الإلغاء. تنبيه: إن كان الطلب قد خرج للتوصيل فقد لا يمكن إلغاؤه.",
  default: "تمام 🙌 حوّلنا محادثتك لموظف من فريق دكانجي وسيردّ عليك قريباً خلال ساعات العمل (٩ صباحاً–١١ مساءً بتوقيت إسطنبول).\nلتسريع المتابعة اكتب مشكلتك بالتفصيل مع رقم الطلب إن وُجد."
};
