...
OUTB = os.path.join(ROOT, "assets", "photos", "afgan")
...
def wix(url, w=640, h=640, fit="fill"):
    # append a Wix media transform to a raw media url
    if "/v1/" in url:
        return url
    fname = url.rstrip("/").split("/")[-1]
    return f"{url}/v1/{fit}/w_{w},h_{h},al_c,q_85/{fname}"
...
def fetch(url):
    ...
    if os.path.exists(out):
        return True
    try:
        data = urllib.request.urlopen(urllib.request.Request(wix(url), headers=UA), timeout=30).read()
...
# logo (transparent cutout png)
logo_url = "https://static.wixstatic.com/media/75699a_df4841267c5d4b79834d8c0bbbb184d4~mv2_d_2627_2127_s_2.png/v1/fit/w_700,h_700,al_c,q_90/file.png"
...
imap = {url: f"/assets/photos/afgan/products/{slug}.webp" for url, slug in urls.items()}
json.dump(imap, open(os.path.join(ROOT, "scripts", "afgan_imagemap.json"), "w", encoding="utf-8"), ensure_ascii=False)

# cover
CW, CH = 1600, 600
cover = Image.new("RGB", (CW, CH))
top, bot = (40, 26, 14), (16, 10, 5)
dr = ImageDraw.Draw(cover)
for y in range(CH):
    tt = y/CH
    dr.line([(0, y), (CW, y)], fill=tuple(int(top[k]*(1-tt)+bot[k]*tt) for k in range(3)))
picks = []
for w in ["Barbecue", "Pilav Rice", "Platter", "Classics"]:
    for p in products:
...
for f, (x, y, s) in zip(picks, [(70, 150, 300), (310, 90, 235), (310, 330, 235)]):
    ...
    dr.ellipse([x-5, y-5, x+s+5, y+s+5], fill=(206, 150, 66))
