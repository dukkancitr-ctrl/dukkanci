// One-time insert of store 118 (سما ماركت) + its 98 products into Supabase via
...
const { semamarketStore, semamarketProducts } = require('../semamarket-data.js');
...
function toDbStore(s) {
  return {
    id: s.id, name: s.name, slug: 'sema-market-esenyurt', category: s.category, image: s.image, cover_image: s.coverImage,
...
(async () => {
  ...
  const chk1 = await (await fetch(BASE + '/rest/v1/stores?select=id&id=eq.' + semamarketStore.id, { headers: H })).json();
  const ids = semamarketProducts.map(p => p.id);
  const chk2 = await (await fetch(BASE + '/rest/v1/products?select=id&id=gte.' + Math.min(...ids) + '&id=lte.' + Math.max(...ids) + '&limit=1', { headers: H })).json();
  const chk3 = await (await fetch(BASE + '/rest/v1/stores?select=id&slug=eq.sema-market-esenyurt', { headers: H })).json();
  if (chk1.length || chk2.length || chk3.length) throw new Error('ABORT: id/slug collision ' + JSON.stringify([chk1, chk2, chk3]));
  const storeRow = toDbStore(semamarketStore);
  const productRows = semamarketProducts.map(toDbProduct);
