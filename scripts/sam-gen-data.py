...
imap = json.load(open(os.path.join(ROOT, "scripts", "sam_imagemap.json"), encoding="utf-8"))
...
STORE = {
    ...
    "featured": True,
    "hasOffer": False,
    "description": "مطعم أيام شامية — نكهات شامية أصيلة: الشاورما بأنواعها، الفروج والوجبات الغربية، كاسات الحلويات والبوظة والوافل، إضافة إلى العصائر والكوكتيلات الطازجة والأطباق الشرقية.",
...
for idx, p in enumerate(products):
    ...
    fpath = os.path.join(ROOT, img.lstrip("/").replace("/", os.sep)) if img else ""
    if not img or not os.path.exists(fpath):
        continue  # only products with real images
    ...
    catalog.append({
        ...
        "available": True,
        "featured": idx < 6,
        "description": f"{p['name']} من مطعم أيام شامية، يُحضَّر طازجاً.",
        "imageFit": "cover",
        "options": [],
    })

js = lambda o: json.dumps(o, ensure_ascii=False, indent=1)
out = f"""// Generated for مطعم أيام شامية (Sam Gunleri) — Esenyurt, Istanbul. Source: samgunleri.com (image-bearing items only)
...
print("wrote sam-data.js:", len(catalog), "products (image-bearing only)")
