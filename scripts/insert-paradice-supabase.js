// One-time insert of store 104 (بردايس) + its 171 products into Supabase via
...
const { paradiceStore, paradiceProducts } = require('../paradice-data.js');
...
(async () => {
  const storeRow = toDbStore(paradiceStore);
  const productRows = paradiceProducts.map(toDbProduct);
  ...
  // Chunk products (171 rows) to keep request bodies reasonable.
  const chunkSize = 60;
  for (let i = 0; i < productRows.length; i += chunkSize) {
    ...
    console.log('products chunk', i, 'status', r2.status, 'count', Array.isArray(s2) ? s2.length : JSON.stringify(s2).slice(0, 400));
