// One-time insert of store 103 (بروستد عبد الحميد) + its 52 products into Supabase via
...
const { abdulhamitStore, abdulhamitProducts } = require('../abdulhamit-data.js');
...
(async () => {
  const storeRow = toDbStore(abdulhamitStore);
  const productRows = abdulhamitProducts.map(toDbProduct);
