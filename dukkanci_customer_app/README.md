...
### v2.1 (2026-07-16) — اختبار حقيقي على محاكي + 4 أعطال إضافية
أُنشئ محاكي (`dukkanci_test`، Pixel 7، أندرويد 35) واختُبر التطبيق فعلياً عليه.
**تدفّق الطلب تحقّق منه بالكامل حياً لأول مرة**: طلب حقيقي `DK-4172774138` وصل
جدول `orders` بكل الحقول صحيحة (`status='طلب جديد'`, `delivery_details` jsonb
بالهاتف/الدفع/العنوان/الأصناف). 4 أعطال إضافية اكتُشفت بالتشغيل الفعلي:

| العطل | الجذر |
|---|---|
| السلة تُنسى عند إغلاق التطبيق | `build()` كان يستدعي `_restore()` غير متزامنة تضبط `state` ثم يُعيد سلة فارغة **تطمسها** (لا `await` داخلها فالترتيب معكوس) |
| المنطقة المختارة تُنسى كل تشغيل | `LocationController` كان في الذاكرة فقط → صار يُحفظ في `LocalCache` + السبلاش يقفز للرئيسية مباشرة |
| **شاشة السلة فارغة تماماً (P0 — يمنع الشراء)** | شريط الملخّص كان يلفّ نص الإجمالي بـ`Expanded` داخل `Row`؛ الشريط بأكمله (والقائمة فوقه) توقّف عن الرسم **بلا أي استثناء**. عُزل بالتنصيف على المحاكي؛ أُعيد بناؤه بنفس نمط `product_screen.dart` المُثبَت (الزر هو المرن، والنص يأخذ حجمه) |
| انقطاع واتساب كان سيرفض **كل** الطلبات | `sendOtp` كان يرمي دائماً؛ الموقع (`startCheckoutOtp`) يمرّر الطلب عند `soft` → أُضيف `OtpSendResult.skippedWhatsappUnavailable` |

**درس منهجي:** `flutter analyze` نظيف + بناء ناجح **لا يعنيان شيئاً** عن صحة
التشغيل — كل هذه الأعطال الأربعة مرّت من الفحص الثابت بلا أي تحذير.

### v2 (2026-07-16) — إصلاح أعطال المخطط + إعادة تصميم
راجع الجدول أعلاه.
...
- **الطلب يُحفظ فعلاً (تحقّق حي ✅) لكن إشعار واتساب للمتجر لم يُختبر** — الاختبار جرى ضد خادم محلي بلا مفاتيح واتساب عمداً (حتى لا يصل إشعار وهمي لتاجر حقيقي)، فمسار `sendOrderWhatsapp` نفسه لم يُنفَّذ. الحفظ في `orders` مؤكَّد.
- 🔴 **`source` يُسجَّل `'web'` لكل طلبات التطبيق** — نقطة `/api/notify-order` المنشورة حالياً لا تقرأ `body.source` إطلاقاً فتقع على قيمة العمود الافتراضية؛ أي أن طلبات الأندرويد **لا يمكن تمييزها** عن طلبات الموقع في التقارير. الحل يأتي مع نقطة `create-order` الجديدة (تقبل `body.source`) التي تبنيها جلسة أخرى حالياً.
- 🔴 **رسوم التوصيل غير محسوبة** (الإجمالي = المجموع الفرعي) و**السعر يُرسَل من العميل** — المواصفات (قسم 14) توجب التحقق من السعر خادمياً. نقطة `create-order` الجديدة تفعل ذلك بالضبط؛ **يجب ترحيل `OrderRepository` إليها فور نشرها**.
